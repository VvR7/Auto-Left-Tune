...
OUTPUT_MIDI = "autodl-tmp/打上花火合奏_topk.mid"
VOCAB_SIZE = 410
BOS_ID = 0
EOS_ID = 1
PAD_ID = 2
...
# ========== 生成函数 ==========
@torch.no_grad()
def topk_generate(model, src, bos_id, eos_id, max_len=8000, pad_id=2, top_k=10, temperature=1.0):
    model.eval()
    ...
    for _ in range(max_len):
        ...
        logits = model.output_layer(out[:, -1])  # 取最后一位的预测
        next_token = top_k_sampling(logits.squeeze(0), k=top_k, temperature=temperature)
...
# ========== 3. 生成左手 ==========
generated_tokens = topk_generate(model, src_tensor, bos_id=BOS_ID, eos_id=EOS_ID,
                                 max_len=MAX_LEN, pad_id=PAD_ID, top_k=TOP_K, temperature=TEMPERATURE)
...
print(f"✅ Top-k 生成完成：{OUTPUT_MIDI}")
